...
public class Account {
    ...
    public Account(int accountNumber, String name, int age, double initialBalance, String accountType) {
        ...
        if (initialBalance < minimumBalance) {
            throw new IllegalArgumentException(accountType + " account requires minimum balance of ₹" + minimumBalance + ". Provided: ₹" + initialBalance);
        }
    ...
    public void deposit(double amount) throws InvalidAmountException, InactiveAccountException {
        validateActive();
        if (amount <= 0) throw new InvalidAmountException("Deposit amount must be positive. Provided: ₹" + amount);
        balance += amount;
    }
    ...
    public void withdraw(double amount, int pin) throws InvalidAmountException, InsufficientBalanceException,
        ...
        if (amount <= 0) throw new InvalidAmountException("Withdrawal amount must be positive. Provided: ₹" + amount);
        if (amount > balance) throw new InsufficientBalanceException(
                "Insufficient balance. Available: ₹" + balance + ", Requested: ₹" + amount);
        double newBalance = balance - amount;
        if (newBalance < getMinimumBalance()) throw new MinimumBalanceViolationException(
    ...
    private double getMinimumBalance() { return getMinimumBalance(accountType); }
    private double getMinimumBalance(String accountType) {
        return accountType.equals("Current") ? MIN_BALANCE_CURRENT : MIN_BALANCE_SAVINGS;
    }
