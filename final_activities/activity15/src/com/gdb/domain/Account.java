...
public abstract class Account implements IAccount {
    ...
    public Account(int accountNumber, String name, int age, double initialBalance, int tenureYears) throws InvalidAgeException {
        if (age < 18) throw new InvalidAgeException("Customer must be at least 18 years old. Provided: " + age);
        if (name == null || name.trim().isEmpty()) throw new InvalidAgeException("Name cannot be empty");
    ...
    public void deposit(double amount) throws InactiveAccountException, InvalidAmountException {
        ...
        if (amount <= 0) throw new InvalidAmountException("Deposit amount must be positive. Provided: Rs. " + amount);
        balance += amount;
    }
    ...
    public void closeAccount() throws InactiveAccountException {
        if (!"Active".equals(status)) throw new InactiveAccountException("Account is already closed / inactive.");
        status = "Inactive";
    }

    @Override
    public void reopenAccount() throws InactiveAccountException {
        if ("Active".equals(status)) throw new InactiveAccountException("Account is already active.");
        status = "Active";
    }

    @Override
    public void setPin(int pin) throws InvalidPinException {
        if (pin < 1000 || pin > 9999) throw new InvalidPinException("PIN must be a 4-digit number (1000-9999). Provided: " + pin);
        this.pin = pin;
    }
    ...
    public String getAccountInfo() {
        return "Account #" + accountNumber + " | " + accountHolderName + " (" + age + " yrs, Tenure: " + tenureYears + " yrs) | " + getAccountType() + " | Rs. " + balance + " | " + status;
    }
